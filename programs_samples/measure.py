...
for f in range(0, 10):
    print ("Checking fault injection run " + str(f) + "...", end="\r")
    file_out = std_output + "/std_outputfile-run-1-" + str(f)
    try:
        file_err = open(errdir + "/errorfile-run-1-" + str(f))
        error_msg = file_err.read()
        file_err.close()
    except FileNotFoundError:   # no error output
        error_msg = ""
...
for f in range(0, 10):
    print ("Checking fault injection run " + str(f) + "...", end="\r")
    file_out = std_output + "/std_outputfile-run-2-" + str(f)
    try:
        file_err = open(errdir + "/errorfile-run-2-" + str(f))
        error_msg = file_err.read()
        file_err.close()
    except FileNotFoundError:   # no error output
        error_msg = ""
